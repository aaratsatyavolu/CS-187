	...
	private static void makeCycleGraph(boolean reverse, boolean double_edges) {
		...
		for (int i = 0; i < nverts; i++) {
			...
			if (!reverse) {
				...
				if (double_edges)
					mat[i][(i + 3) % nverts] = true;
			} else {
				mat[i][(i - 2 + nverts) % nverts] = true;
				if (double_edges)
					mat[i][(i - 3 + nverts) % nverts] = true;
			}
		}
...
//
//	public static void testSimpleBFS() {
//		makeCycleGraph(false, true);
//		System.out.print("BFS in a cycle graph: ");
//		graphS.BFS(0);
//	}
	...
	public static void testBFS() {
		...
		makeCycleGraph(false, true);
		ArrayList<Integer> path = graphS.BFS(0, 1);
		if (path != null) {
			for (Integer x : path) {
				System.out.print(x + " ");
			}
