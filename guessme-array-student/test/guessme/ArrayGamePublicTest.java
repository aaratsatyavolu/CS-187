...
public class ArrayGamePublicTest {
	...
	public void testNumMatchesTrivial() {
		assertEquals("0 match", 0, ArrayGame.numMatches(1234, 4321));
		assertEquals("0 match", 0, ArrayGame.numMatches(1234, 2341));
	}

	@Test(timeout = 1000)
	public void testNumMatches() {
		assertEquals("1 matches", 1, ArrayGame.numMatches(5678, 7777));
		assertEquals("1 match", 1, ArrayGame.numMatches(9000, 3210));

		assertEquals("2 matches", 2, ArrayGame.numMatches(1234, 1114));
		assertEquals("2 matches", 2, ArrayGame.numMatches(5005, 6006));

		assertEquals("3 matches", 3, ArrayGame.numMatches(1000, 9000));

		assertEquals("4 matches", 4, ArrayGame.numMatches(1234, 1234));
	}
	...
	@Test(timeout = 1000)
	public void testFirstGuessTwo() {
		gamerB.reset();
		assertEquals("test first guess", 1000, gamerB.getGuess());
	}
	...
	public void testPriorGuesses() {
		...
		assertArrayEquals("test prior guesses", new int[] { g1 }, gamerB.priorGuesses());
		...
		assertArrayEquals("test prior guesses", new int[] { g1, g2, g3, g4 }, gamerB.priorGuesses());
		gamerB.updateGuess(4);
		assertArrayEquals("test prior guesses", new int[] { g1, g2, g3, g4 }, gamerB.priorGuesses());
	}
...
//
//		int[] checks = {2000, 5000, 9000 };
//		for (int i = 0; i < checks.length; i++) {
//			int number = checks[i];
//			int n1 = gamerC.getGuess();
//
//			int nm = gamerC.numMatches(n1, number);
//			while (nm != 4) {
//				gamerC.updateGuess(nm);
//				n1 = gamerC.getGuess();
//				nm = gamerC.numMatches(n1, number);
//			}
//			assertEquals("prior guess is equal to target:", number, gamerC.getLastGuess());
//			gamerC.reset();
//		}
//		System.out.println(Arrays.toString(gamerC.getAllGuesses()));
//	}

}

/*
 * gamerA= new game() number = 5000; n1 =gamerA.getGuess(); nm=
 * numatch(n1,number); while nm !=4 { n1 =gamerA.getGuess(); nm=
 * numatch(n1,number);}
 * 
 * 
 */
