...
public class RedBlackTree<T extends Comparable<T>> extends BinarySearchTree<T> {
	...
	// 3. if parent node is red, keep doing recursively for BST property by
	// following,
	...
	private void verifyColor(BSTNode<T> node) {
		if (node == root) {
			node.setColor(BSTNode.BLACK);
			return;
		}
		if (node.getParent().getColor() == BSTNode.BLACK) {
			return;
		}
		if (node.getParent().getColor() == BSTNode.RED && node.getUncle() != null) {
			if (node.getUncle().getColor() == BSTNode.RED) {
				node.getUncle().setColor(BSTNode.BLACK);
				node.getParent().setColor(BSTNode.BLACK);
				node.getGrandParent().setColor(BSTNode.RED);
				verifyColor(node.getGrandParent());
			} else if (node.getUncle().getColor() == BSTNode.BLACK && node.isOutside()) {
				node.getParent().setColor(BSTNode.BLACK);
				node.getGrandParent().setColor(BSTNode.RED);
				if (node == node.getParent().getRight() && node.getParent() == node.getGrandParent().getRight())
				...
				verifyColor(node.getParent());
			} else if (node.getUncle().getColor() == BSTNode.BLACK && node.isInside()) {
				if (node == node.getParent().getRight() && node.getParent() == node.getGrandParent().getLeft()) {
					rotateLeft(node.getParent());
					verifyColor(node.getParent());
				}
				if (node == node.getParent().getLeft() && node.getParent() == node.getGrandParent().getRight()) {
	...
	private void recover(BSTNode<T> parent) {
		...
		while ((node == null || isBlack(node)) && (node != this.root)) {
			if (parent.getLeft() == node) {
				...
				// Case 2: if brother node's color is black and both its children's color are
				// also black,
	...
	// In the end, if the removed node's color is black, call recover method on its
	// parent node.
	@Override
